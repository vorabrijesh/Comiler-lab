...
void postorder(struct Snode *ptr) {
	...
	if (ptr->left!=NULL && ptr->right!=NULL && strcmp(ptr->lexval,"="))
	{
		...
		vector<string> ww ;
		ww.push_back(ptr->reg) ;
		ww.push_back("=");
		ww.push_back(ptr->left->reg);
		ww.push_back(ptr->lexval);
		ww.push_back(ptr->right->reg);
		res.push_back(ww);
		if( (ptr->tok!="NUM"  && ptr->reg[0]!='t') )
		{
			int flag1=0;
			for(int k=0;k<v.size();k++)
			{
				if(v[k].count(ptr->reg))
					flag1=1;
			}
			if(flag1==1)
				printf("\nError: var %s is not declared in the scope",ptr->reg);			
		} 
		if(strcmp(ptr->left->tok,"NUM")  && ptr->left->reg[0]!='t')
		{
			int flag1=0;
			for(int k=0;k<v.size();k++)
			{
				if(v[k].count(ptr->left->reg))
					flag1=1;
			}
			if(flag1==1)
				printf("\nError: var %s is not declared in the scope",ptr->left->reg);
		}
		if(strcmp(ptr->right->tok,"NUM")  && ptr->right->reg[0]!='t')
		{
			int flag1=0;
			for(int k=0;k<v.size();k++)
			{
				if(v[k].count(ptr->right->reg))
					flag1=1;
			}
			if(flag1==1)
